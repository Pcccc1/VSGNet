...
class RGBXDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        item_name = item_name.split("/")[1].split(".png")[0]
        rgb_path = os.path.join(
            self._rgb_path,
            item_name.replace(".jpg", "").replace(".png", "") + self._rgb_format,
        )
        x_path = os.path.join(
            self._x_path,
            item_name.replace(".jpg", "").replace(".png", "") + self._x_format,
        )
        gt_path = os.path.join(
            self._gt_path,
            item_name.replace(".jpg", "").replace(".png", "") + self._gt_format,
        )
