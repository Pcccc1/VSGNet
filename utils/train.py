...
from model.builder import EncoderDecoder as segmodel
...
from model.losses import DiceLoss, SoftCrossEntropyLoss
...
with Engine(custom_parser=parser) as engine:
    ...
    Diceloss_fn = DiceLoss(ignore_index=config.background)
    SoftCrossEntropy_fn = SoftCrossEntropyLoss(smooth_factor=0.1, ignore_index=config.background)
    #criterion = nn.CrossEntropyLoss(reduction='mean', ignore_index=config.background)
    criterion = L.JointLoss(first=Diceloss_fn, second=SoftCrossEntropy_fn, first_weight=0.5, second_weight=0.5)
    ...
    for epoch in range(engine.state.epoch, config.nepochs+1):
        ...
        if (epoch%1==0 and epoch>=int(config.checkpoint_start_epoch and epoch % config.checkpoint_step==0)) or epoch==1:
            ...
            if engine.distributed:
                ...
                if miou>best_miou and miou>50:
                    with open("res.txt", "w") as f:
                        f.write(acc, macc, f1, mf1, ious, miou)
                    best_miou=miou
                    engine.save_and_link_checkpoint(config.checkpoint_dir,
                                                    config.log_dir,
                                                    config.log_dir_link)
