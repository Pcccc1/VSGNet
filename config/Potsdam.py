...
C.dataset_path = osp.join(C.root_dir, "Potsdam")
...
C.x_root_folder = osp.join(C.dataset_path, "")
C.x_format = ".png"

C.train_source = osp.join(C.dataset_path, "train.txt")
C.eval_source = osp.join(C.dataset_path, "test.txt")
C.is_test = True
C.num_train_imgs = 6589
C.num_eval_imgs = 4393
C.num_classes = 5
C.class_names = [
    "surfaces",
    "building",
...
C.eval_crop_size = [640, 640]  # [height weight]
